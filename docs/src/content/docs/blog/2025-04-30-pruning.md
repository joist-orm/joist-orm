...
// When users make a web request like:
// `GET /authors?name=foo&publisherName=bar&bookStatus=published&highRating=true`
// We assume our web framework calls `getAuthors` with `filter` set as the decoded query string
...
async function getAuthors(filter: AuthorsQueryString): Promise<Author[]> {
  const { publisherId, publisherName, bookStatusId, highRating } = filter;
  return await db
    .selectDistinct(authors)
    .from(authors)
    .innerJoin(publishers, eq(authors.publisherId, publishers.id))
