...
* We include a dummy `1 = 1` so that all our conditional predicates can always use an `AND` prefix.
  ...
  Without this hack, we'd have to dynamically track "what is the 1st predicate that needs `AND` added".
...
...wait a second, this is example the same code?

## Joist's Trick: Usage Tracking

Joist's realization is that, for a majority of "dynamic query" use cases, the _structure_ of the query is usually extremely stable, it's just that *parts of the structure* are turned on/off based on conditional input.

Unfortunately, most query DSLs cannot "turn off" parts of the query, and so it's left to _our code_ to handle the boilerplate of "turn this off" by fundamentally "changing the structure" via tedious `if` checks, ternary operators, or altering our chained methods (often the most complicated, in my experience).
...
So far we've looked at pruning "inline conditions", which are predicates inlined directly in joins--all of these inline conditions are "AND"s.
