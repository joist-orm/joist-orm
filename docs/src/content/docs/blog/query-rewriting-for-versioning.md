...
We've discovered a scheme that will "makes reads automatically version-aware"--now we want our application to use it, basically all the time, without us messing up or forgetting the rewrite incantation.

Given this seems like a very mechanical translation, **what if we could automate it?**

Our application already does all reads through Joist, as `em.load` or `em.find` calls:
