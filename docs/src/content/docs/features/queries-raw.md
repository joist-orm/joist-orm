...
const titles = b.title.arrayAgg({
  distinct: true,
  orderBy: [{ expr: b.title, direction: "DESC", nulls: "first" }],
...
The **expression form** takes arbitrary expressions — a column, an aggregate, or a `sql` template — including fields you didn't select. Each entry has an `expr`, a `direction: "ASC" | "DESC" | undefined`, and an optional `nulls: "first" | "last"`:

```ts
orderBy: [
  { expr: b.id.count(), direction: "DESC" },
  { expr: a.first_name, direction: "ASC", nulls: "last" },
];
```
...
```ts
orderBy: [{ bookCount: "DESC" }, { expr: a.first_name, direction: "ASC", nulls: "last" }];
```

Both forms allow `undefined` entries. A keyed direction of `undefined` prunes that key; an expression direction of `undefined` prunes the complete entry before its expression references are collected, so it does not retain an otherwise-unused join. Prefer the keyed form whenever what you're ordering by is already in `select`. [Set operations](#set-operations) only support the keyed form at their root.
...
const firstAuthorId = query({
  from: ids,
  select: ids.id,
  orderBy: [{ expr: ids.id, direction: "ASC" }],
  limit: 1,
}).coalesce("a:1");
// Expr<AuthorId, never>
```
