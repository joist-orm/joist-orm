...
`with` adds CTEs to a `WITH` clause before the statement, the same clause [`em.query`](/features/queries-raw/#ctes-with) takes. The CTE is in scope for the whole statement: an `UPDATE`/`DELETE` `where`, an `INSERT`'s `SELECT` source, a `VALUES` cell, and `returning`.
