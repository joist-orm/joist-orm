...
/** Renders an expression order for either a query or an aggregate, or prunes an undefined direction. */
export function orderByToSql(o: ExpressionOrderBy, ctx: ExprContext): SqlFragment | undefined {
  if (!(o.expr instanceof BaseExpr)) {
    return fail("orderBy must be an expression, i.e. a table column, aggregate, sql`...`, or scalar query(...)");
  }
  if (Object.keys(o).some((key) => key !== "expr" && key !== "direction" && key !== "nulls")) {
    return fail("Expression orderBy entries only accept expr, direction, and nulls");
  }
  if (o.direction === undefined) return undefined;
  if (o.direction !== "ASC" && o.direction !== "DESC") {
    return fail(`Invalid orderBy direction '${o.direction}'`);
  }
  ...
  const fragment = o.expr.toSql(ctx);
  const nulls = o.nulls ? ` NULLS ${o.nulls.toUpperCase()}` : "";
  return { ...fragment, sql: `${fragment.sql} ${o.direction}${nulls}` };
}
