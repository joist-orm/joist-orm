...
- `find/` owns entity-shaped filters, domain aliases, scopes, find parsing, join optimization, SQL rendering, and find dataloaders.
...
EntityManager coordinates execution, permissions, hydration, and plugins. Database access and entity writes live in `drivers/`. Public exports are declared in `src/index.ts`; implementation modules import each other directly.
