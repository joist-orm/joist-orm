...
/** An expression order-by entry. An undefined direction prunes the complete entry. */
export type ExpressionOrderBy = {
  readonly expr: ExprLike<unknown>;
 ...
 * in `select`, mix in `{ expr, direction }` entries in the array form.
 */
...
function orderBysToSql(q: AnyQuery, ctx: Ctx): SqlFragment[] {
  ...
  for (const entry of Array.isArray(orderBy) ? orderBy : [orderBy]) {
    if (entry === undefined) continue;
    // A select key can also be named expr, direction, or nulls, so distinguish by the expr value.
    if (isExpr(entry.expr)) {
