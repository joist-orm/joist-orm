...
export function generateQueryResolvers(db: DbMetadata): CodegenFile[] {
  const resolvers = db.entities.map((e) => {
    ...
    return {
      name: `resolvers/${camelName}/${camelName}Resolver.ts`,
      overwrite: false,
      contents: code`
  ...
  const testFiles = db.entities.map((e) => {
    ...
    const camelName = camelCase(name);
    const resolverConst = imp(`${camelName}@src/resolvers/${camelName}/${camelName}Resolver`);
    return {
      name: `resolvers/${camelName}/${camelName}Resolver.test.ts`,
      overwrite: false,
      contents: code`
        describe("${camelName}", () => {
