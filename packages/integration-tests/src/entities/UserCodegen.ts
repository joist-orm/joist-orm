import { BaseEntity, Changes, cleanStringValue, Collection, ConfigApi, EntityFilter, EntityGraphQLFilter, EntityMetadata, EntityOrmField, fail, FilterOf, Flavor, GraphQLFilterOf, hasMany, hasManyToMany, hasOne, isLoaded, Lens, Loaded, LoadHint, loadLens, ManyToOneReference, newChangesProxy, newRequiredRule, OptsOf, OrderBy, PartialOrNull, setField, setOpts, ValueFilter, ValueGraphQLFilter } from "joist-orm";
import { Context } from "src/context";
import { IpAddress, PasswordValue } from "src/entities/types";
import { Author, AuthorId, authorMeta, AuthorOrder, Comment, CommentId, commentMeta, newUser, User, userMeta } from "./entities";
...
export abstract class UserCodegen extends BaseEntity<EntityManager> {
  ...
  readonly createdComments: Collection<User, Comment> = hasMany(commentMeta, "createdComments", "user", "user_id", undefined);
  readonly authorManyToOne: ManyToOneReference<User, Author, undefined> = hasOne(authorMeta, "authorManyToOne", "userOneToOne");
  readonly likedComments: Collection<User, Comment> = hasManyToMany("users_to_comments", "likedComments", "liked_by_user_id", commentMeta, "likedByUsers", "comment_id");
  ...
  * */
  ...
  setPartial(opts: PartialOrNull<UserOpts>): void {
    setOpts(((this as any) as User), (opts as OptsOf<User>), {
      partial: true
    });
  }
