...
export function loadDataLoader<T extends Entity>(
  ...
  return em.getLoader("load", meta.type, (loads) => loadBatchFn(em, meta, loads));
}

type Load<T extends Entity> = { entity: string; hint: LoadHint<T> | undefined };

export async function loadBatchFn<T extends Entity>(
  em: EntityManager,
  meta: EntityMetadata,
  loads: readonly Load<T>[],
  ...
  const entities: T[] = em.hydrate(meta.cstr, rows, { overwriteExisting: true });
