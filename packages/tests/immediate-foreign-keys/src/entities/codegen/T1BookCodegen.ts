...
import { EntityManager, newT1Book, T1Author, t1AuthorMeta, T1Book, t1BookMeta } from "../entities";
import type { Entity, T1AuthorId, T1AuthorOrder } from "../entities";
...
export abstract class T1BookCodegen extends BaseEntity<EntityManager, number> implements Entity {
  ...
  get author(): ManyToOneReference<T1Book, T1Author, never> {
    return this.__data.relations.author ??= hasOne(this as any as T1Book, t1AuthorMeta, "author", "t1Books");
  }
}
