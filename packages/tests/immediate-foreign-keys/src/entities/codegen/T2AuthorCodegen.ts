...
import { EntityManager, newT2Author, T2Author, t2AuthorMeta, T2Book, t2BookMeta } from "../entities";
import type { Entity, T2BookId, T2BookOrder } from "../entities";
...
export abstract class T2AuthorCodegen extends BaseEntity<EntityManager, number> implements Entity {
  ...
  get t2Books(): Collection<T2Author, T2Book> {
    return this.__data.relations.t2Books ??= hasMany(
      this as any as T2Author,
      t2BookMeta,
      "t2Books",
      "author",
      "author_id",
