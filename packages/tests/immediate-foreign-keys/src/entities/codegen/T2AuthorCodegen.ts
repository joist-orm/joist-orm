...
export interface T2AuthorColumns {
  "id": { type: IdOf<T2Author>; entity: T2Author; nullable: false; insert: "optional"; update: false };
  "first_name": { type: string; nullable: false; insert: "required"; update: true };
  "favorite_book_id": { type: IdOf<T2Book>; entity: T2Book; nullable: true; insert: "optional"; update: true };
}
...
export const t2AuthorScope = newScopeFn<T2Author, T2AuthorScope>("T2Author");
...
export abstract class T2AuthorCodegen extends BaseEntity<EntityManager, number> implements Entity {
  ...
  readonly t2Books: Collection<T2Author, T2Book> = hasMany();
  readonly favoriteBook: ManyToOneReference<T2Author, T2Book, undefined> = hasOne();
