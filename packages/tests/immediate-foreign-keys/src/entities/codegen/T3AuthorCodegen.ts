...
export const t3AuthorScope = newScopeFactory<T3Author>("T3Author");
...
export abstract class T3AuthorCodegen extends BaseEntity<EntityManager, number> implements Entity {
  ...
  readonly t3Books: Collection<T3Author, T3Book> = hasMany();
  readonly favoriteBook: ManyToOneReference<T3Author, T3Book, never> = hasOne();
