...
export interface T3BookColumns {
  ...
  "author_id": { kind: "m2o"; type: T3Author; derived: false; nullable: false; insert: "required"; update: true };
}
...
export abstract class T3BookCodegen extends BaseEntity<EntityManager, number> implements Entity {
  ...
  readonly t3Authors: Collection<T3Book, T3Author> = hasMany();
  readonly author: ManyToOneReference<T3Book, T3Author, never> = hasOne();
