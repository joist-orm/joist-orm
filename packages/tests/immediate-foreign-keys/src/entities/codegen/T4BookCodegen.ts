...
export interface T4BookColumns {
  "id": { type: IdOf<T4Book>; entity: T4Book; nullable: false; insert: "optional"; update: false };
  "title": { type: string; nullable: false; insert: "required"; update: true };
  "author_id": { type: IdOf<T4Author>; entity: T4Author; nullable: false; insert: "required"; update: true };
}
...
export interface T4BookScopes {
}

export type T4BookScope = Scope<T4Book, T4BookScopes>;
...
export abstract class T4BookCodegen extends BaseEntity<EntityManager, number> implements Entity {
  ...
  readonly t4Authors: Collection<T4Book, T4Author> = hasMany();
  readonly author: ManyToOneReference<T4Book, T4Author, never> = hasOne();
