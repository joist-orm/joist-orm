...
export interface T5BookScopes {
}

export type T5BookScope = Scope<T5Book, T5BookScopes>;

export const t5BookConfig = new ConfigApi<T5Book, Context>();

export const t5BookScope = newScopeFn<T5Book, T5BookScope>("T5Book");
...
export abstract class T5BookCodegen extends BaseEntity<EntityManager, number> implements Entity {
  ...
  readonly reviews: Collection<T5Book, T5BookReview> = hasMany();
  readonly author: ManyToOneReference<T5Book, T5Author, never> = hasOne();
