import { type EntityManager, ExactColumnsPlugin, type ExactColumnsTrackOutcome, MissingColumnError } from "joist-orm";
...
describe("ExactColumnsPlugin", () => {
  it("narrows after the profile union stays stable for three invocations", async () => {
    ...
    expect(outcomes).toEqual(
      Array(4).fill({
        endpointKey: "get-author",
        optimized: false,
        retried: false,
        missingColumnFailure: false,
        columnsBefore: 37,
        columnsOmitted: 0,
      }),
    );
    // The 5th invocation narrows to `firstName` + the always-kept pk/timestamp columns.
    resetQueryCount();
    await invoke(plugin, "get-author", endpoint);
  ...
  it("resets stability when a learning invocation discovers a new field", async () => {
    // Given an author with two fields that will be discovered in separate invocations.
    await insertAuthor({ first_name: "a1", last_name: "l1" });
    // And an endpoint that initially reads only firstName.
    const { plugin, outcomes } = observe();
    ...
    await invoke(plugin, "get-author", (em) => endpoint(em, false));
    expect(outcomes.at(-1)?.optimized).toBe(false);

    // And reading lastName grows the union before the original settling window completes.
  ...
  it("batches narrowed finds with explicit column group bys", async () => {
    // Given two authors selected by separate logical finds in one SQL batch.
    await insertAuthor({ first_name: "a1" });
    // And a second author with a distinct firstName filter.
    await insertAuthor({ first_name: "a2" });
    // And an endpoint that records both primary projections.
    const { plugin, outcomes } = observe();
    async function endpoint(em: EntityManager): Promise<void> {
  ...
  it("counts logical finds even when a repeated find uses the cache", async () => {
    // Given an author selected twice with the same filter.
    await insertAuthor({ first_name: "a1" });
    // And an endpoint whose second find reuses the first find's cached result.
    const { plugin, outcomes } = observe();
    /** Reads the same author through two logical primary projections. */
    async function endpoint(em: EntityManager): Promise<void> {
      const [a] = await em.find(Author, { firstName: "a1" });
      expect(a.firstName).toBe("a1");
      const [cached] = await em.find(Author, { firstName: "a1" });
      expect(cached).toBe(a);
    }
    await settle(plugin, "cached", endpoint);
    expect(outcomes).toEqual(
      Array(4).fill({
        endpointKey: "cached",
        optimized: false,
    ...
    resetQueryCount();
    await invoke(plugin, "cached", endpoint);
    expect(outcomes.at(-1)).toEqual({
      endpointKey: "cached",
      optimized: true,
      retried: false,
  ...
  it("retries novel field reads and backs off after repeated missing-column errors", async () => {
    // Given an author with fields reached by progressively novel branches.
    await insertAuthor({ first_name: "a1", last_name: "l1", age: 40 });
    // And callbacks that record missing-column errors and each completed invocation.
    const missingColumns: MissingColumnError[] = [];
    const outcomes: ExactColumnsTrackOutcome[] = [];
    const plugin = new ExactColumnsPlugin({
      onMissingColumn: (err) => missingColumns.push(err),
      onTrack: (outcome) => outcomes.push(outcome),
    });
    ...
    // The narrowed invocation takes a novel branch; track sees the missing-column error, widens, and re-invokes with full rows.
    resetQueryCount();
    ...
    `);
    expect(missingColumns).toMatchObject([{ fieldName: "lastName", endpointKey: "get-author" }]);
    ...
    `);
    expect(outcomes.at(-1)).toMatchObject({ optimized: true, retried: false });
    ...
    for (let i = 0; i < 100; i++) {
      await invoke(plugin, "get-author", (em) => endpoint(em, false));
      expect(outcomes.at(-1)?.optimized).toBe(true);
    }
    // And a third novel field now needs eight stable calls, not thirteen, because one fault decayed.
    await invoke(plugin, "get-author", async (em) => (await em.load(Author, "a:1")).ssn);
    expect(outcomes.at(-1)?.retried).toBe(true);
    for (let i = 0; i < 7; i++) {
      await invoke(plugin, "get-author", (em) => endpoint(em, false));
      expect(outcomes.at(-1)?.optimized).toBe(false);
    }
  ...
  it("keeps throwing when user code swallows the missing-column error", async () => {
    await insertAuthor({ first_name: "a1", last_name: "l1" });
    const plugin = new ExactColumnsPlugin();
    async function endpoint(em: EntityManager, readLast: boolean): Promise<void> {
      const a = await em.load(Author, "a:1");
      ...
      if (readLast) {
        ...
        expect(() => a.lastName).toThrow(MissingColumnError);
  ...
  it("flushes narrowed entities without clobbering unfetched columns", async () => {
    ...
    expect(outcomes.at(-1)).toMatchObject({ optimized: true, retried: false, missingColumnFailure: false });
  });

  it("does not auto-retry a missing-column error after a committed flush", async () => {
    await insertAuthor({ first_name: "a1", last_name: "l1", age: 40 });
    const missingColumns: MissingColumnError[] = [];
    const outcomes: ExactColumnsTrackOutcome[] = [];
    const plugin = new ExactColumnsPlugin({
    ...
    async function endpoint(em: EntityManager, firstName: string, readAge: boolean): Promise<void> {
      ...
      // a missing-column error after the flush above has already committed
    ...
    await expect(invoke(plugin, "rename-author", (em) => endpoint(em, "a3", true))).rejects.toThrow(MissingColumnError);
    // The write that preceded the missing-column error stayed committed, i.e. the failure did not roll it back
    expect(await select("authors")).toMatchObject([{ id: 1, first_name: "a3", last_name: "l1" }]);
    expect(missingColumns).toMatchObject([{ fieldName: "age", endpointKey: "rename-author" }]);
    expect(outcomes).toHaveLength(5);
    expect(outcomes.at(-1)).toMatchObject({ optimized: true, retried: false, missingColumnFailure: true });
    ...
    expect(outcomes.at(-1)).toMatchObject({ optimized: false, retried: false, missingColumnFailure: false });
  });
  ...
  it("guards forked partial entities", async () => {
    ...
    async function endpoint(em: EntityManager, fork: boolean): Promise<void> {
      ...
      if (fork) {
        ...
        expect(() => forked.lastName).toThrow(MissingColumnError);
      }
    }
    await settle(plugin, "get-author", (em) => endpoint(em, false));
    await invoke(plugin, "get-author", (em) => endpoint(em, true));
  ...
  it("converges em.clone in one retry", async () => {
    ...
    async function endpoint(em: EntityManager, clone: boolean): Promise<void> {
      const a = await em.load(Author, "a:1");
      expect(a.firstName).toBe("a1");
      // Cloning reads every field, so the narrowed invocation throws one missing-column error, then retries with full rows.
      if (clone) expect((await em.clone(a)).lastName).toBe("l1");
    }
    await settle(plugin, "get-author", (em) => endpoint(em, false));
    await invoke(plugin, "get-author", (em) => endpoint(em, true));
    expect(outcomes).toHaveLength(5);
    expect(outcomes.at(-1)).toMatchObject({ optimized: true, retried: true, missingColumnFailure: false });
  });
  ...
  it("composes with lazy columns", async () => {
    ...
    async function endpoint(em: EntityManager, key: string): Promise<void> {
      ...
      // Blind-writing an unloaded lazy column must not cause a false missing-column error
      pg.bulkData.set({ key });
      await em.flush();
    }
  ...
  it("hydrates populated children fully without false missing-column errors", async () => {
    await insertAuthor({ first_name: "a1" });
    await insertBook({ title: "b1", author_id: 1 });
    const { plugin, outcomes } = observe();
    async function endpoint(em: EntityManager): Promise<void> {
    ...
    expect(outcomes).toHaveLength(5);
    expect(outcomes.at(-1)).toMatchObject({ retried: false, missingColumnFailure: false });
  });

  it("reports once when an invocation issues no queries", async () => {
    // Given an endpoint that never loads an entity.
    const { plugin, outcomes } = observe();
    resetQueryCount();
    for (let i = 0; i < 5; i++) {
      await expect(plugin.track("empty", async () => "result")).resolves.toBe("result");
    }
  ...
  it("reports ordinary errors without retrying or replacing them", async () => {
    // Given an author loaded before an application error.
    await insertAuthor({ first_name: "a1" });
    // And a callback that records failed invocations.
    const { plugin, outcomes } = observe();
    const error = new Error("application failed");
    await expect(
      invoke(plugin, "failure", async (em) => {
    ...
    // And the same endpoint now has a settled firstName projection before another ordinary failure.
    await settle(plugin, "failure", async (em) => (await em.load(Author, "a:1")).firstName);
    outcomes.length = 0;
    await expect(
      invoke(plugin, "failure", async (em) => {
        expect((await em.load(Author, "a:1")).firstName).toBe("a1");
        throw error;
      }),
    ).rejects.toBe(error);
    expect(outcomes).toEqual([
  ...
  it("reports both attempts once when the full-row retry fails", async () => {
    // Given a settled endpoint that has only read firstName.
    await insertAuthor({ first_name: "a1", last_name: "l1" });
    // And an outcome callback installed before learning.
    const { plugin, outcomes } = observe();
    await settle(plugin, "retry-failure", async (em) => (await em.load(Author, "a:1")).firstName);
    // And the new lastName branch fails with an ordinary error after its missing-column error triggers a retry.
    const error = new Error("retry failed");
    outcomes.length = 0;
    let attempts = 0;
    await expect(
      invoke(plugin, "retry-failure", async (em) => {
        attempts++;
        expect((await em.load(Author, "a:1")).lastName).toBe("l1");
        throw error;
      }),
    ).rejects.toBe(error);
  ...
  it("swallows synchronous outcome callback errors without changing results or retries", async () => {
    // Given an author whose lastName will be read only after learning.
    await insertAuthor({ first_name: "a1", last_name: "l1" });
    // And telemetry that throws on every invocation.
    const outcomes: ExactColumnsTrackOutcome[] = [];
    const plugin = new ExactColumnsPlugin({
      onTrack: (outcome) => {
    ...
    await settle(plugin, "throwing-hook", async (em) => (await em.load(Author, "a:1")).firstName);
    await expect(invoke(plugin, "throwing-hook", async (em) => (await em.load(Author, "a:1")).lastName)).resolves.toBe(
      "l1",
    );
    expect(outcomes).toHaveLength(5);
    expect(outcomes.at(-1)?.retried).toBe(true);
    // And an application failure must still escape unchanged.
    const error = new Error("application failed");
    await expect(
      plugin.track("throwing-hook", async () => {
        throw error;
      }),
    ).rejects.toBe(error);
    expect(outcomes).toHaveLength(6);
    expect(outcomes.at(-1)).toEqual({
      endpointKey: "throwing-hook",
      optimized: false,
      retried: false,
      missingColumnFailure: false,
      columnsBefore: 0,
  ...
  it("isolates concurrent invocations of the same endpoint while one retries", async () => {
    // Given a settled firstName endpoint shared by concurrent requests.
    await insertAuthor({ first_name: "a1", last_name: "l1" });
    // And a callback that records completion order.
    const { plugin, outcomes } = observe();
    await settle(plugin, "concurrent", async (em) => (await em.load(Author, "a:1")).firstName);
    // And the first request pauses after its narrowed load until the second request retries.
    const loaded = new Deferred<void>();
    const release = new Deferred<void>();
    outcomes.length = 0;
    const first = invoke(plugin, "concurrent", async (em) => {
      const a = await em.load(Author, "a:1");
      loaded.resolve();
    ...
    try {
      await expect(invoke(plugin, "concurrent", async (em) => (await em.load(Author, "a:1")).lastName)).resolves.toBe(
        "l1",
      );
      expect(outcomes).toEqual([
        {
          endpointKey: "concurrent",
          optimized: true,
          retried: true,
          missingColumnFailure: false,
          columnsBefore: 74,
  ...
  it("counts lazy primary projections without counting on-demand lazy loads", async () => {
    // Given a ParentGroup with lazy data outside its four-column primary projection.
    await insertParentGroup({ name: "pg1", bulk_data: { key: "value" }, required_data: {} });
    // And an endpoint that only reads its timestamp and lazy data, leaving name prunable.
    const { plugin, outcomes } = observe();
    /** Loads lazy data separately from the primary ParentGroup projection. */
    async function endpoint(em: EntityManager): Promise<void> {
      const pg = await em.load(ParentGroup, "parentGroup:1");
      expect(pg.createdAt).toBeInstanceOf(Date);
      expect(await pg.bulkData.load()).toEqual({ key: "value" });
    }
    await settle(plugin, "lazy", endpoint);
    expect(outcomes).toEqual(
      Array(4).fill({
        endpointKey: "lazy",
        optimized: false,
    ...
    await invoke(plugin, "lazy", endpoint);
    expect(outcomes.at(-1)).toEqual({
      endpointKey: "lazy",
      optimized: true,
      retried: false,
...
/** Records public outcomes without exposing the plugin's learned state. */
function observe(): { plugin: ExactColumnsPlugin; outcomes: ExactColumnsTrackOutcome[] } {
  const outcomes: ExactColumnsTrackOutcome[] = [];
  const plugin = new ExactColumnsPlugin({ onTrack: (outcome) => outcomes.push(outcome) });
  return { plugin, outcomes };
}
