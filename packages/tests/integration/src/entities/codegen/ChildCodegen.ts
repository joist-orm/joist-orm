...
export interface ChildScopes {
}

export type ChildScope = Scope<Child, ChildScopes>;

export const childConfig = new ConfigApi<Child, Context>();

export const childScope = newScopeFn<Child, ChildScope>("Child");
...
export abstract class ChildCodegen extends BaseEntity<EntityManager, string> implements Entity {
   ...
   * @see {@link https://joist-orm.io/features/partial-update-apis | Partial Update APIs} on the Joist docs
   */
  setDeepPartial(opts: DeepPartialOrNull<Child>): Promise<void> {
    return updatePartial(this as any as Child, opts);
  }
