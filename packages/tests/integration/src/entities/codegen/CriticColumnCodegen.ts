...
export abstract class CriticColumnCodegen extends BaseEntity<EntityManager, string> implements Entity {
   ...
   * @see {@link https://joist-orm.io/features/partial-update-apis | Partial Update APIs} on the Joist docs
   */
  setDeepPartial(opts: DeepPartialOrNull<CriticColumn>): Promise<void> {
    return updatePartial(this as any as CriticColumn, opts);
  }
  ...
  get critic(): ManyToOneReference<CriticColumn, Critic, never> {
    return this.__data.relations.critic ??= (hasOne(this, criticMeta, "critic", "criticColumn") as any).create(
      this,
      "critic",
    );
  }
