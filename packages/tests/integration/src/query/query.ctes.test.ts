...
describe("em.query / ctes", () => {
  ...
  it("adds a CTE for a set query's operands", async () => {
    // Given Author a1
    await insertAuthor({ first_name: "a1" });
  ...
  describe("mutations", () => {
    it("adds a CTE for an UPDATE", async () => {
    ...
    it("adds a CTE for a DELETE", async () => {
      // Given Author a1, who has a Book
      await insertAuthor({ id: 1, first_name: "a1" });
  ...
  describe("recursive", () => {
    it("walks a self-referential tree", async () => {
      ...
      const rows = await em.query({
        ...
        from: tree,
        select: tree,
        orderBy: [{ expr: tree.id, direction: "ASC" }],
      });
    ...
    it("stops a cycle with union distinct", async () => {
      ...
      const rows = await em.query({
        ...
        from: chain,
        select: chain,
        orderBy: [{ expr: chain.id, direction: "ASC" }],
      });
