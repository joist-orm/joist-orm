...
import { type JoinTree, type Query, type Subquery, alias, query, queryMaybe, table, tables } from "joist-orm";
...
function treeTypeAssertions() {
  ...
  expectTypeOf(em.query({ from: a, join: broadTree, select: a.firstName })).toEqualTypeOf<Promise<(string | null)[]>>();
