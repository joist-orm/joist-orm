...
describe("em.query", () => {
  ...
  describe("select shapes", () => {
    ...
    it("escapes quotes in subquery as names, in declarations and references", async () => {
      ...
      const column = await em.query({
        from: evil,
        select: { n: evil.name },
        orderBy: [{ sort: evil.name, order: "ASC" }],
      });
  ...
  describe("aggregates", () => {
    ...
    it("prunes an arrayAgg order and its otherwise-unused join when order is undefined", async () => {
      ...
      // When collecting Author names with an undefined Book title order
      resetQueryCount();
      const rows = await em.query({
        from: a,
        join: [a.books.inner(b)],
  ...
  describe("ordering and paging", () => {
    ...
    it("rejects an expression orderBy without sort", async () => {
      // Given an aggregate expression order without its required sort expression
      const em = newEntityManager();
      const [a] = tables(Author);
    ...
    it("prunes an expression order and its otherwise-unused join when order is undefined", async () => {
    ...
    it("orders selected keys that match expression order properties as keyed array entries", async () => {
      ...
      expect(rows).toEqual([
        { asc: 20, desc: "a1", nulls: "a:2", sort: 20, order: "a1" },
        { asc: 20, desc: "a1", nulls: "a:1", sort: 20, order: "a1" },
        { asc: 20, desc: "a2", nulls: "a:3", sort: 20, order: "a2" },
        { asc: 10, desc: "a0", nulls: "a:4", sort: 10, order: "a0" },
      ]);
      expect(queries).toMatchInlineSnapshot(`
       [
         "SELECT a.age AS "asc", a.first_name AS "desc", a.id AS nulls, a.age AS sort, a.first_name AS "order" FROM authors AS a WHERE a.deleted_at IS NULL ORDER BY "asc" DESC, "desc" ASC, nulls DESC, sort DESC, "order" ASC",
       ]
      `);
    });
  ...
  describe("raw sql escape hatches", () => {
    ...
    it("can emulate DISTINCT ON with a ranked subquery", async () => {
      ...
      const rows = await em.query({
        ...
        orderBy: [{ sort: ranked.authorId, order: "ASC" }],
      });
      expect(rows).toEqual([
