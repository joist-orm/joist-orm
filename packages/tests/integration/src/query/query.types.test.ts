...
async function typeAssertions() {
  ...
  const q = {
    ...
    orderBy: [{ name: "ASC" }, { expr: a.age, direction: "ASC" }],
  } satisfies Query;
  ...
  const orderedNames = query({
    from: a,
    select: { name: a.first_name },
    orderBy: [{ expr: a.age, direction: "ASC" }, { name: "ASC" }],
  });
  ...
  // @ts-expect-error: expression ordering requires expr even when direction is present
  em.query({ from: a, select: { name: a.first_name }, orderBy: [{ direction: "ASC" }] });
  // @ts-expect-error: expression directions are uppercase SQL literals
  em.query({ from: a, select: { name: a.first_name }, orderBy: [{ expr: a.age, direction: "asc" }] });
  ...
  em.query({ from: a, select: { name: a.first_name }, orderBy: [{ expr: a.age, direction: "ASC" }, { age: "ASC" }] });
  ...
  em.query({ from: a, select: { name: a.first_name }, orderBy: [{ expr: a.age, name: "ASC", direction: "ASC" }] });
  const unionSelect: Query<{ name: typeof a.first_name } | { age: typeof a.age }> = {
    ...
    orderBy: [{ expr: a.age, name: "ASC", direction: "ASC" }],
  };
