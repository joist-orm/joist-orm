...
describe("em.query / where", () => {
  ...
  it("omits undefined range bounds without dropping defined bounds", async () => {
    ...
    const base = { from: a, select: a.first_name, orderBy: [{ expr: a.first_name, direction: "ASC" }] } as const;
  ...
  it("filters owning references without joining their target tables", async () => {
    ...
    const b = table(Book);
    const base = { from: b, select: b.title, orderBy: [{ expr: b.title, direction: "ASC" }] } as const;
