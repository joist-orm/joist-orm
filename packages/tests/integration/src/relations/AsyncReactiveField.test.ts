  ...
  it("issues UPDATE after initial INSERT with calculated value", async () => {
    ...
    expect(queries).toMatchInlineSnapshot(`
     [
       ...
       "WITH _find (tag, arg0) AS (SELECT unnest($1::int[]), unnest($2::character varying[])) SELECT array_agg(_find.tag) as _tags, a.* FROM authors AS a CROSS JOIN _find AS _find WHERE a.deleted_at IS NULL AND a.last_name = _find.arg0 GROUP BY a.id ORDER BY a.id ASC LIMIT $3",
       "BEGIN;",
       "WITH data AS (SELECT unnest($1::int[]) as id, unnest($2::character varying[]) as name, unnest($3::decimal[]) as latitude, unnest($4::decimal[]) as longitude, unnest($5::decimal[]) as huge_number, unnest($6::int[]) as number_of_book_reviews, unnest($7::timestamp with time zone[]) as deleted_at, unnest($8::text[]) as titles_of_favorite_books, unnest($9::text[]) as book_advance_titles_snapshot, unnest($10::text[]) as number_of_book_advances_snapshot, unnest($11::text[]) as base_sync_default, unnest($12::text[]) as base_async_default, unnest($13::timestamp with time zone[]) as created_at, unnest($14::timestamp with time zone[]) as updated_at, unnest($15::text[]) as favorite_author_name, unnest($16::int[]) as rating, unnest($17::int[]) as size_id, unnest($18::int[]) as type_id, unnest($19::int[]) as favorite_author_id, unnest($20::int[]) as group_id, unnest($21::int[]) as spotlight_author_id) INSERT INTO publishers (id, name, latitude, longitude, huge_number, number_of_book_reviews, deleted_at, titles_of_favorite_books, book_advance_titles_snapshot, number_of_book_advances_snapshot, base_sync_default, base_async_default, created_at, updated_at, favorite_author_name, rating, size_id, type_id, favorite_author_id, group_id, spotlight_author_id) SELECT * FROM data",
       "WITH data AS (SELECT unnest($1::int[]) as id, unnest($2::text[]) as shared_column, unnest($3::text[]) as country) INSERT INTO large_publishers (id, shared_column, country) SELECT * FROM data",
       "WITH data AS (SELECT unnest($1::int[]) as id, unnest($2::character varying[]) as first_name, unnest($3::character varying[]) as last_name, unnest($4::character varying[]) as ssn, unnest($5::character varying[]) as initials, unnest($6::int[]) as number_of_books, unnest($7::text[]) as book_comments, unnest($8::boolean[]) as is_popular, unnest($9::int[]) as age, unnest($10::date[]) as graduated, unnest_arrays($11::character varying[][], true) as nick_names, unnest_arrays($12::character varying[][], true) as nick_names_upper, unnest($13::boolean[]) as was_ever_popular, unnest($14::boolean[]) as is_funny, unnest($15::text[]) as mentor_names, unnest($16::text[]) as mentee_names, unnest($17::jsonb[]) as address, unnest($18::jsonb[]) as business_address, unnest($19::jsonb[]) as quotes, unnest($20::bigint[]) as number_of_atoms, unnest($21::timestamp with time zone[]) as deleted_at, unnest($22::int[]) as number_of_public_reviews, unnest($23::int[]) as "numberOfPublicReviews2", unnest($24::character varying[]) as tags_of_all_books, unnest($25::text[]) as search, unnest($26::text[]) as image_file_name, unnest($27::bytea[]) as certificate, unnest($28::timestamp with time zone[]) as created_at, unnest($29::timestamp with time zone[]) as updated_at, unnest($30::favorite_shape[]) as favorite_shape, unnest($31::int[]) as range_of_books, unnest_arrays($32::int[][], true) as favorite_colors, unnest($33::int[]) as current_draft_book_id, unnest($34::int[]) as favorite_book_id, unnest($35::int[]) as mentor_id, unnest($36::int[]) as publisher_id, unnest($37::int[]) as root_mentor_id) INSERT INTO authors (id, first_name, last_name, ssn, initials, number_of_books, book_comments, is_popular, age, graduated, nick_names, nick_names_upper, was_ever_popular, is_funny, mentor_names, mentee_names, address, business_address, quotes, number_of_atoms, deleted_at, number_of_public_reviews, "numberOfPublicReviews2", tags_of_all_books, search, image_file_name, certificate, created_at, updated_at, favorite_shape, range_of_books, favorite_colors, current_draft_book_id, favorite_book_id, mentor_id, publisher_id, root_mentor_id) SELECT * FROM data",
       "WITH data AS (SELECT unnest($1::int[]) as id, unnest($2::character varying[]) as title, unnest($3::int[]) as "order", unnest($4::text[]) as notes, unnest($5::text[]) as acknowledgements, unnest($6::text[]) as authors_nick_names, unnest($7::text[]) as search, unnest($8::timestamp with time zone[]) as deleted_at, unnest($9::timestamp with time zone[]) as created_at, unnest($10::timestamp with time zone[]) as updated_at, unnest($11::int[]) as author_id, unnest($12::int[]) as prequel_id, unnest($13::int[]) as random_comment_id, unnest($14::int[]) as reviewer_id) INSERT INTO books (id, title, "order", notes, acknowledgements, authors_nick_names, search, deleted_at, created_at, updated_at, author_id, prequel_id, random_comment_id, reviewer_id) SELECT * FROM data",
