  ...
  it("skips UPDATE after INSERT if value hasn't changed", async () => {
    ...
    expect(queries).toMatchInlineSnapshot(`
     [
       ...
       "WITH data AS (SELECT unnest($1::int[]) as id, unnest($2::character varying[]) as first_name, unnest($3::character varying[]) as last_name, unnest($4::character varying[]) as ssn, unnest($5::character varying[]) as initials, unnest($6::int[]) as number_of_books, unnest($7::text[]) as book_comments, unnest($8::boolean[]) as is_popular, unnest($9::int[]) as age, unnest($10::date[]) as graduated, unnest_arrays($11::character varying[][], true) as nick_names, unnest_arrays($12::character varying[][], true) as nick_names_upper, unnest($13::boolean[]) as was_ever_popular, unnest($14::boolean[]) as is_funny, unnest($15::text[]) as mentor_names, unnest($16::text[]) as mentee_names, unnest($17::jsonb[]) as address, unnest($18::jsonb[]) as business_address, unnest($19::jsonb[]) as quotes, unnest($20::bigint[]) as number_of_atoms, unnest($21::timestamp with time zone[]) as deleted_at, unnest($22::int[]) as number_of_public_reviews, unnest($23::int[]) as "numberOfPublicReviews2", unnest($24::character varying[]) as tags_of_all_books, unnest($25::text[]) as search, unnest($26::bytea[]) as certificate, unnest($27::timestamp with time zone[]) as created_at, unnest($28::timestamp with time zone[]) as updated_at, unnest($29::favorite_shape[]) as favorite_shape, unnest($30::int[]) as range_of_books, unnest_arrays($31::int[][], true) as favorite_colors, unnest($32::int[]) as mentor_id, unnest($33::int[]) as root_mentor_id, unnest($34::int[]) as current_draft_book_id, unnest($35::int[]) as favorite_book_id, unnest($36::int[]) as publisher_id) INSERT INTO authors (id, first_name, last_name, ssn, initials, number_of_books, book_comments, is_popular, age, graduated, nick_names, nick_names_upper, was_ever_popular, is_funny, mentor_names, mentee_names, address, business_address, quotes, number_of_atoms, deleted_at, number_of_public_reviews, "numberOfPublicReviews2", tags_of_all_books, search, certificate, created_at, updated_at, favorite_shape, range_of_books, favorite_colors, mentor_id, root_mentor_id, current_draft_book_id, favorite_book_id, publisher_id) SELECT * FROM data",
       "INSERT INTO author_to_mentees_closure (mentor_id, mentee_id) VALUES ($1, $2) ON CONFLICT (mentor_id, mentee_id) DO UPDATE SET id = author_to_mentees_closure.id RETURNING id;",
