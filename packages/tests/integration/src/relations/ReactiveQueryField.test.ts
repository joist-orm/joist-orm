  ...
  it("skips UPDATE after INSERT if value hasn't changed", async () => {
    ...
    expect(queries).toMatchInlineSnapshot(`
     [
       ...
       "begin ",
       "WITH data AS ( SELECT unnest($1::int[]) as id, unnest($2::character varying[]) as name, unnest($3::decimal[]) as latitude, unnest($4::decimal[]) as longitude, unnest($5::decimal[]) as huge_number, unnest($6::int[]) as number_of_book_reviews, unnest($7::timestamp with time zone[]) as deleted_at, unnest($8::text[]) as titles_of_favorite_books, unnest($9::text[]) as base_sync_default, unnest($10::text[]) as base_async_default, unnest($11::timestamp with time zone[]) as created_at, unnest($12::timestamp with time zone[]) as updated_at, unnest($13::int[]) as rating, unnest($14::int[]) as size_id, unnest($15::int[]) as type_id, unnest($16::int[]) as favorite_author_id, unnest($17::int[]) as group_id, unnest($18::int[]) as spotlight_author_id ) INSERT INTO publishers (id, name, latitude, longitude, huge_number, number_of_book_reviews, deleted_at, titles_of_favorite_books, base_sync_default, base_async_default, created_at, updated_at, rating, size_id, type_id, favorite_author_id, group_id, spotlight_author_id) SELECT * FROM data",
