...
export class PostgresTestDriver implements TestDriver {
  ...
  constructor() {
    this.sql = postgres({
      ...
      onquery: () => {
        return (q: any) => recordQuery(q.strings.join("?"));
      },
    } as any);
    this.driver = new PostgresDriver(this.sql);
  }
  ...
  select(tableName: string): Promise<readonly any[]> {
    return this.sql`select * from ${tableName} order by id`;
  }
  ...
  async insert(tableName: string, row: Record<string, any>, subclassTable = false): Promise<void> {
    if (row.id && !subclassTable) {
      ...
      await this.sql`SELECT setval('${tableName}_id_seq', ${row.id}, true)`;
    }
    await this.sql`insert into ${tableName} ${this.sql(row, Object.keys(row))}`;
  }

  async update(tableName: string, row: Record<string, any>): Promise<void> {
    const columns = Object.keys(row).filter((k) => k !== "id");
    await this.sql`update ${tableName} set ${this.sql(row, columns)} where id = ${row.id}`;
  }

  async delete(tableName: string, id: number): Promise<void> {
    await this.sql`delete from ${tableName} where id = ${id}`;
  }

  async count(tableName: string): Promise<number> {
    return (await this.sql`select count(*) as count from ${tableName}`)[0].count;
  }
}
