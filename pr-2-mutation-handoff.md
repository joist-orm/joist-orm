...
- Generated field types carry a type-only `columns: [{ nullable; insert: "required" | "optional" | "never"; update }]` tuple policy for the single storage column, using the same `notNull` convention and existing timestamp mapping. For supported non-inherited mutation targets, `notNull` reflects column nullability. STI can strengthen `notNull` for domain fields; generated tuples, runtime `sqlNullable`, and array serde flags follow that existing convention, not an independently preserved physical inheritance invariant. STI and CTI mutations remain excluded. `execute.ts` and `Aliases.ts` infer policy from this tuple. Runtime mutation checks read `field.serde.columns[0]`, with timestamp omission from `EntityMetadata.timestampFields`; codegen uses `meta.createdAt`/`meta.updatedAt`.
